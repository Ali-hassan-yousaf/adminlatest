  ...
  const handleDeletePrecaution = async (id) => {
    ...
    try {
      const response = await fetch(`https://pet-well-zuxu.vercel.app/api/worker/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ shopname: "Deleted" }), // Update shopname to "Deleted"
      });

      if (!response.ok) {
        const data = await response.json();
        setErrorMessage(data.message || "Failed to soft delete precaution.");
        setLoading(false);
        return;
      }

      // Remove the precaution from the state as it no longer matches shopname === "Precautions"
      setPrecautions(precautions.filter((precaution) => precaution._id !== id));
    } catch (error) {
      console.error("Error soft deleting precaution:", error);
      setErrorMessage("An unexpected error occurred.");
    }
    setLoading(false);
  };
